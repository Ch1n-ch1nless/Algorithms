...
HamiltonianPath FindHamiltonianPathWithMinCost(
    std::vector<std::vector<unsigned int>>& graph) {
  size_t max_bit_mask = (1 << graph.size()) - 1;

  std::vector<std::vector<unsigned int>> path_cost(
      graph.size(), std::vector<unsigned int>(max_bit_mask + 1, INF));
  std::vector<std::vector<unsigned int>> prev(
      graph.size(), std::vector<unsigned int>(max_bit_mask + 1, INF));
  ...
  std::reverse(result.vertex_sequence.begin(), result.vertex_sequence.end());
...
int main() {
  ...
  std::vector<std::vector<unsigned int>> graph(
      vertex_num, std::vector<unsigned int>(vertex_num, 0));
