...
class SuffixAutomaton {
 ...
 public:
  ...
  int64_t countUniqueSubstrings() {
    int64_t result = 0;
    for (size_t i = 1; i < states_.size(); ++i) {
      result += states_[i].longest_ - states_[states_[i].suffix_link_].longest_;
    }
    return result;
    }
 ...
 private:
  ...
  int64_t countPaths(int64_t id, std::vector<bool>& used) {
    if (used[id]) { return 0; }
    ...
    for (auto pair : states_[id].transitions_) {
      int64_t next = pair.second;
      count += countPaths(next, used);
    }
...
int main() {
  ...
  uint64_t answer = suffix_automation.countUniqueSubstrings();
  std::cout << answer << "\n";
